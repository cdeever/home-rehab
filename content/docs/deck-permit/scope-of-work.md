...
- Dimensions: 13'-10" × 13'-10" (matches existing footprint — unchanged in reconstruction).
...
- An existing ½" (nominal) gas stub passes through a notch in the existing ledger. **This stub has not been in service — the owner has used a propane grill throughout ownership — so it is effectively abandoned-in-place.** It will be decommissioned and capped in the crawl space rather than relocated; see Work To Be Performed step 11. The new ledger will be continuous with no utility penetrations.
...
5. **Ledger.** Install new 2×8 PT (UC4A) ledger with a **positive mechanical connection to the house floor framing** (not to sheathing alone). Attach to the solid sawn lumber rim joist — owner to verify rim is sawn 2× lumber (not OSB/LVL rim board) and in sound condition before installation; replace any rot-compromised rim lumber first. Fasten with ½" diameter HDG through-bolts with washers at spacing per IRC Table R507.9.1.3(1) for the actual 11'-7" joist span. Install continuous corrosion-resistant metal drip cap / Z-flashing over the ledger top and step flashing where the ledger meets wall cladding, integrated with the existing WRB. Additionally install (2) Simpson DTT2Z lateral load tension ties per IRC R507.9.2, located near the ends of the ledger, tying the deck joists through the rim joist into interior floor framing per the Franklin County Deck Guide detail.
6. **Joists.** Install 2×8 PT SP #2 joists at 16" o.c., face-mounted to the ledger with Simpson LUS28 hangers. Joists bear fully on top of the beam and are mechanically secured with Simpson H2.5A hurricane ties for uplift resistance. Joists cantilever 2'-3" past the beam to the outer edge. At the two clipped corners, joists terminate at the angled rim joist via Simpson LSSU210 skewed hangers; see Framing Plan for details.
...
9. **Stairs and landing.** Install a single run of stairs off the east (right) side of the deck, carrying traffic from deck level down to grade:
    ...
    - **Stringers:** (3) 2×12 Southern Pine #2 PT stringers, notched per IRC R311.7.3. Top of stringers hung from deck rim with Simpson LSC stair stringer connectors. At the bottom, stringers sit on a PT 2×6 kicker plate anchored to the compacted aggregate base **beneath** the paver landing — pavers are the finish surface only, not structural bearing. Kicker anchored to the base with HDG concrete screws; galvanized angle brackets tie stringers to the kicker.
...
11. **Decommission unused gas stub (by licensed plumber, separate permit).** The gas line currently notched through the ledger has never been placed in service by the current owner — a propane grill has been used throughout ownership. Under a separate Franklin County Public Health mechanical/gas permit, a licensed Ohio plumbing contractor (per ORC § 4740) will:
    ...
    - Cap the remaining interior line in the crawl space per IFGC 406 (plugged/capped gas outlets), and leak-test the capped branch.
    - Remove any exterior line components so that when the new ledger is installed, no utility penetrations exist at the deck-to-house interface.

    The deck permit does not authorize gas work; this step is documented here so the reviewer sees that the ledger notch is being eliminated — not merely hidden by new framing — and that no new gas piping is being introduced as part of the deck reconstruction.
...
- Target start: _TBD, pending permit approval_
