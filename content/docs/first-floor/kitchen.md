---
title: "Kitchen"
weight: 1
tasks_completed: 58
tasks_in_progress: 1
tasks_planned: 0
---

# Kitchen

## Blinds
- ✅ Install blinds - 3 bay and 1 sink window <!-- small -->

## Painting
- ✅ Repaint ceiling <!-- small -->
- ✅ Patch, prime and repaint walls <!-- medium -->

## Electrical
- ✅ Replace outlets and switches (white) <!-- small -->
- ✅ Replace above sink light <!-- quick -->
- ✅ Install dedicated outlet for microwave <!-- small -->

## Plumbing
- ✅ Run new fridge ice/water line <!-- small -->
- ✅ Replace saddle valve tap with proper copper and add local shutoff valve <!-- small -->
- ✅ Replace sink <!-- small -->
- ✅ Replace sink fixtures and add soap dispenser <!-- small -->

## Mechanicals
- ✅ Replace garbage disposal <!-- quick -->

## Cabinets
- ✅ Prep and repaint cabinets <!-- large -->
- ✅ Replace cabinet hinges with hidden style and add pulls <!-- small -->

## Backsplash
- ✅ Design <!-- medium -->
- ✅ Education/training <!-- large -->
- ✅ Install tile <!-- medium -->
- ✅ Apply sealant <!-- quick -->
- 🔄 Seal around border edges <!-- quick -->

## Countertops
- ✅ Replace counter tops <!-- small -->

## Appliances
- ✅ Install above oven microwave <!-- small -->
- ✅ Replace dishwasher <!-- small -->
- ✅ Replace oven <!-- quick -->
- ✅ Replace fridge <!-- small -->
- ✅ Reinstall dishwasher kickplate <!-- quick -->

## Pantry
- ✅ Repaint ceiling <!-- small -->
- ✅ Patch, prime and repaint walls <!-- medium -->
- ✅ Repaint door and casing <!-- small -->
- ✅ Replace knobs and hinges <!-- quick -->
